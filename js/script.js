...
function toSec(date) {
	...
	for (var i = 0; i < 6; i++) time += date[i] * secs[i];
	return time;
}
...
function countdown() {
	...
	var secstart = toSec(startdate),
		secspected = toSec(specteddate);
	...
	document.getElementById('progressbar1').innerHTML =
		elapsed[1] + 1 + ' months      ' + elapsed[2] + ' days';
	document.getElementById('progressbar2').style.width = 90 - percent + '%';
	document.getElementById('progressbar2').innerHTML =
		remaining[1] - 1 + ' months      ' + remaining[2] + ' days';
}
